...
public:
  enum {
    ...
    ERROR_TEMP2 = 0x000004, // Error: Temperature Sensor 2 >=100C (available
                            // only on some models)
    ...
    ERROR_FAULTM1 =
        0x000040, // Error: Motor 1 Driver Fault (only on some models)
    ERROR_FAULTM2 =
        0x000080, // Error: Motor 2 Driver Fault (only on some models)
    ...
    WARN_TEMP2 = 0x200000, // Warning: Temperature Sensor 2 >=85C (available
                           // only on some models)
...
public:
  ...
  ~RoboClaw();
  uint32_t ReadM1AverageSpeed(uint8_t address);
